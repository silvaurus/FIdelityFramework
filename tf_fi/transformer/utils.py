...
def get_tensor(num_batches, num_samples, sess, tensor, layer_list, dict):
    hypotheses = []
    for _ in range(num_batches):
        h, ly = sess.run([tensor, layer_list])
        print('Here are the layers!')
        print('{} - {}'.format(layer_list[0], ly[0].shape))
        print('{} - {}'.format(layer_list[1], ly[1].shape))
        print('{} - {}'.format(layer_list[2], ly[2].shape))

        #print('{} - {}'.format(layer_list[0], ly[0][0][0]))
        #print('{} - {}'.format(layer_list[1], ly[1][0][0]))
        #print('{} - {}'.format(layer_list[2], ly[2][0][0]))

        #print(layer_list[0][0][0])
        #print(layer_list[1][0][0])
        #print(layer_list[2][0][0])
        ...
        for elem in ly[4]:
            print(elem)
        '''
        np.savetxt('/home/yiizy/tf_transformer/new_target_out.txt', ly[0].flatten())
        np.savetxt('/home/yiizy/tf_transformer/new_input_1_out.txt', ly[1].flatten())
        np.savetxt('/home/yiizy/tf_transformer/new_input_2_out.txt', ly[2].flatten())

        '''
        for i in range(len(ly)):
            print('{}'.format(layer_list[i]))
            print('{} - {}'.format(layer_list[i], ly[i].shape))
        '''
...
def get_hypotheses(num_batches, num_samples, sess, tensor, dict,
        delta_inj=None, delta_perturb=None):
    '''Gets hypotheses.
    num_batches: scalar.
    num_samples: scalar.
    ...
    hypotheses = []
    for _ in range(num_batches):
        h = sess.run(tensor, feed_dict={delta_inj: delta_perturb})
        hypotheses.extend(h.tolist())
    hypotheses = postprocess(hypotheses, dict)
...
def calc_return_bleu(ref, translation):
    '''Calculates bleu score and return the score the upper function 
    ref: reference file path
    translation: model output file path

    Returns
    the BLEU score'''
    get_bleu_score = "perl multi-bleu.perl {} < {} > {}".format(ref, translation, translation + "temp")
    os.system(get_bleu_score)
    bleu_score_report = open(translation + "temp", "r").read()
    print(bleu_score_report)

    score = re.findall("BLEU = ([^,]+)", bleu_score_report)[0]
    os.remove(translation + "temp")
    os.remove(translation)
    return score
