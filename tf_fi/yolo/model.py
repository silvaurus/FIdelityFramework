...
from utils.layer_utils import bound_delta_conv2d, darknet53_body, yolo_block, upsample_layer

class yolov3(object):

    def __init__(self, class_num, anchors, use_label_smooth=False, use_focal_loss=False, batch_norm_decay=0.999, weight_decay=5e-4, use_static_shape=True, bound_type=None, bound=None, inj_layer=None, delta_4d=None):
        ...
        with slim.arg_scope([slim.conv2d, slim.batch_norm], reuse=reuse):
            with slim.arg_scope([slim.conv2d], 
                ...
                with tf.variable_scope('darknet53_body'):
                    route_1, route_2, route_3 = darknet53_body(inputs, bound_type=self.bound_type, bound=self.bound, inj_layer=self.inj_layer, delta_4d=self.delta_4d, batch_norm_params=batch_norm_params)

                with tf.variable_scope('yolov3_head'):
                    inter1, net = yolo_block(route_3, 512, bound_type=self.bound_type, bound=self.bound, inj_layer=self.inj_layer, delta_4d=self.delta_4d, batch_norm_params=batch_norm_params)
                    feature_map_1 = bound_delta_conv2d(net, 3 * (5 + self.class_num), 1,
                                                strides=1, normalizer_fn=None,
                                                activation_fn=None, biases_initializer=tf.zeros_initializer(), fixed_padding=False, bound_type=self.bound_type, bound=self.bound, inj_layer=self.inj_layer, delta_4d=self.delta_4d, batch_norm_params=batch_norm_params)
                    feature_map_1 = tf.identity(feature_map_1, name='feature_map_1')

                    inter1 = bound_delta_conv2d(inter1, 256, 1, bound_type=self.bound_type, bound=self.bound, inj_layer=self.inj_layer, delta_4d=self.delta_4d, batch_norm_params=batch_norm_params)
                    inter1 = upsample_layer(inter1, route_2.get_shape().as_list() if self.use_static_shape else tf.shape(route_2))
                    concat1 = tf.concat([inter1, route_2], axis=3)

                    inter2, net = yolo_block(concat1, 256, bound_type=self.bound_type, bound=self.bound, inj_layer=self.inj_layer, delta_4d=self.delta_4d, batch_norm_params=batch_norm_params)
                    feature_map_2 = bound_delta_conv2d(net, 3 * (5 + self.class_num), 1,
                                                strides=1, normalizer_fn=None,
                                                activation_fn=None, biases_initializer=tf.zeros_initializer(), fixed_padding=False, bound_type=self.bound_type, bound=self.bound, inj_layer=self.inj_layer, delta_4d=self.delta_4d, batch_norm_params=batch_norm_params)
                    feature_map_2 = tf.identity(feature_map_2, name='feature_map_2')

                    inter2 = bound_delta_conv2d(inter2, 128, 1, bound_type=self.bound_type, bound=self.bound, inj_layer=self.inj_layer, delta_4d=self.delta_4d, batch_norm_params=batch_norm_params)
                    inter2 = upsample_layer(inter2, route_1.get_shape().as_list() if self.use_static_shape else tf.shape(route_1))
                    concat2 = tf.concat([inter2, route_1], axis=3)

                    _, feature_map_3 = yolo_block(concat2, 128, bound_type=self.bound_type, bound=self.bound, inj_layer=self.inj_layer, delta_4d=self.delta_4d, batch_norm_params=batch_norm_params)
                    feature_map_3 = bound_delta_conv2d(feature_map_3, 3 * (5 + self.class_num), 1,
                                                strides=1, normalizer_fn=None,
                                                activation_fn=None, biases_initializer=tf.zeros_initializer(), fixed_padding=False, bound_type=self.bound_type, bound=self.bound, inj_layer=self.inj_layer, delta_4d=self.delta_4d, batch_norm_params=batch_norm_params)
